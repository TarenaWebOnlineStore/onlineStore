...
window.onload=function(){
    var start=setInterval(autoPlay,5000);//开始轮播，每秒换一张图

    $('#imgchangediv').onmouseover=function(){//当鼠标光标停在图片上，则停止轮播
        clearInterval(start);
    }
    $('#imgchangediv').onmouseout=function(){//当鼠标光标停在图片上，则开始轮播
        start=setInterval(autoPlay,5000);
    }

    //var lis=$('imgyuan').getElementsByTagName('font');//得到所有圆圈
    var lis= document.getElementById("imgyuan").getElementsByTagName('a');//得到所有圆圈
    //当移动到圆圈，则停滞对应的图片
...
function changeImg(index){
    ...
    var list1 = document.getElementById("imgyuan").getElementsByTagName('a');
    //var list1=$('#imgyuan').getElementsByTagName('a');
